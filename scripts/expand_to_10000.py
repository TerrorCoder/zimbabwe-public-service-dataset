#!/usr/bin/env python3
"""
Expansion script to scale Zimbabwe Public Service Knowledge Dataset from 5000 to 10000 records.
Appends SVC-5001 through SVC-10000 into processed/records.jsonl and labels/multilingual_intents.jsonl.
"""
...
sectors_5001_10000 = [
    ...
    "Forestry Charcoal Transport Permit",
...
for idx in range(5001, 10001):
    rec_id = f"SVC-{idx}"
    prov = provinces[(idx - 5001) % len(provinces)]
    dist = districts[(idx - 5001) % len(districts)]
    sect = sectors_5001_10000[(idx - 5001) % len(sectors_5001_10000)]
    
    title = f"{dist} ({prov}) Ward { (idx % 50) + 1 } - {sect} Procedure"
    ...
    rec = {
        ...
        "provenance": {
            ...
            "collection_time": "2026-07-31T23:59:00Z"
        },
...
print(f"Generated {len(new_records)} new records (SVC-5001 to SVC-10000).")
...
print("Successfully appended SVC-5001 to SVC-10000 into records.jsonl and multilingual_intents.jsonl!")
